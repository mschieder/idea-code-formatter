...
public class IdeaCodeFormatterMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException {
        ...
        try {
            new IdeaCodeFormatterEnvironment().format(new IdeaFormatterArgsBuilder().charset(charset).masks(masks).dryRun(dryRun).directories(directories).codestyleSettingsFile(codestyleSettingsFile).build());
        } catch (Exception e) {
